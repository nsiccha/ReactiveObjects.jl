...
"""
    sig(obj::ReactiveObject) -> Symbol

Return the kernel signature symbol (the function name passed to `@reactive`).
For `obj::ReactiveObject{:my_kernel}` this returns `:my_kernel`.
"""
@inline sig(::ReactiveObject{S}) where {S} = S
"""
    valid(obj::ReactiveObject) -> Vector{Bool}

Return the underlying per-field validity flag vector. Index `i` corresponds to
the `i`th tracked field (see [`propertyidx`](@ref) for the field-name → index map).
Reading is cheap; mutating directly bypasses dependency invalidation.
"""
@inline valid(obj::ReactiveObject) = getfield(obj, :valid)
"""
    propertyidx(obj::ReactiveObject, field::Symbol) -> Int

Return the 1-based index of `field` in `obj`'s tracked-property layout — i.e.
its position in [`valid`](@ref)`(obj)` and in the underlying [`data`](@ref)
NamedTuple. The per-field method is generated by [`@reactive`](@ref).
"""
@inline propertyidx(obj::ReactiveObject, x::Symbol) = propertyidx(obj, Val(x))
"""
    data(obj::ReactiveObject) -> NamedTuple

Return the underlying NamedTuple of (possibly `Ref`-wrapped) field storage.
Each entry is either a mutable container (array etc.) or a `Ref` for
immutable scalars. Used internally by `getproperty` / `setproperty!`.
"""
...
"""
    compute!(obj::ReactiveObject, field::Symbol)

Recompute `field` (and its transitive dependencies, in topological order) on
`obj`, marking each as valid. Normally invoked automatically by
`getproperty` when reading an invalid field; call directly to force
evaluation. Dispatches to `compute!(obj, Val(field))`, where the per-field
method is generated by [`@reactive`](@ref).
"""
